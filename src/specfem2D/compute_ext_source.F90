...
!========================================================================
  ...
  subroutine read_ext_source_num()
    ...
    ! hand rank 0's list to everybody else.
    ! v7 did this with a send_i / recv_i loop, one element at a time; v8.1.0 only
    ! provides those point-to-point wrappers in an MPI build, so the serial link
    ! failed. A broadcast is equivalent, exists in both builds, and is O(1) calls
    ! instead of O(NPROC * number_of_extsource).
    call bcast_all_singlei(number_of_extsource)
    call bcast_all_i(iele, number_of_extsource)
  ...
  !-----------------------------------------------------------------------------------
  !
! Everything from here to the end of the file talks to MPI directly and is only
! compiled into an MPI build. The serial build needs none of it: the injection is
! then already single-domain, so there is no interface to average across.
#ifdef WITH_MPI
