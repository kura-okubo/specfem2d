...
!========================================================================
  ...
  subroutine determine_external_source_elements()
    ...
    ! Ported v7.0.0 -> v8.1.0: part_unstruct_par's "nx"/"nz" are now called
    ! nx_elem_internal / nz_elem_internal, and "ngnod" is now NGNOD. Same
    ! meaning: the number of internal-mesher elements along x and z
    ! (2 x nxread / 2 x nzread when NGNOD = 9).
    use part_unstruct_par, only: elmnts, nodes_coords, &
                              nx_elem_internal, nz_elem_internal, nxread, nzread
    ...
    ! write(IMAIN,*)
    !
    ! write(IMAIN,*) "nx_elem_internal, nz_elem_internal ", nx_elem_internal, nz_elem_internal
    ! write(IMAIN,*)
