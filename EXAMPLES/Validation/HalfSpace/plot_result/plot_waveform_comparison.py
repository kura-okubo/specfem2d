...
for i in range(NumofCE, number_of_station):
	...
	plottheta = azimuth[itheta]
	itheta = itheta + 1

	with open("../OUTPUT_FILES_P/AA.S%04d.BXX.sema"%i, 'r') as fi:
		temp_df = pd.read_csv(fi,  header=None, engine='python', sep=r'\s+', comment='#')
...
ax1.set_ylabel(r'Azimuth from north ($^{\circ}$)')
ax2.set_xlabel('Time (s)')
ax2.set_ylabel(r'Azimuth from north ($^{\circ}$)')
...
plt.savefig("validation_half.png", dpi=300)
plt.show()
